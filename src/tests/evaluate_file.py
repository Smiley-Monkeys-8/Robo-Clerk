...
for file_name in list_files_in_folder("out"):
    ...
    result = judge.handcrafted_decision(file_name)
    
    known_result = get_result(client_id)
    if known_result == judge.Decision.Reject and result["consistency_percentage"]<90:
    ...
    if known_result == judge.Decision.Accept and result["consistency_percentage"]>=90:
